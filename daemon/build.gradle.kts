...
listOf("debug", "release").forEach { variantName ->
    ...
    tasks.register<Exec>("buildDaemon$variantCapped") {
        ...
        // cargo-ndk may exit non-zero when no .so is produced (binary crate),
        // but the actual compilation succeeds. We verify the binary in doLast.
        ...
        doLast {
            ...
            if (!src.exists()) {
                throw GradleException("Rust build failed: ${src.absolutePath} not found")
            }
